...
:::note
TCGC emits a `client-default-value-type-mismatch` warning when the type of the value passed to `@clientDefaultValue` does not match the property's scalar type (for example, a string default on an `int32` property). Ensure the default value type matches the property type to avoid this warning. If the mismatch is intentional, you can suppress it:
