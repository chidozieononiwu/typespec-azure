...
### Limitation: Templated parameters with different types

:::warning[Diagnostic: `client-location-conflict`]
When `@clientLocation` is applied to a **templated** parameter (for example, a parameter defined on a template alias that different operations instantiate with different types), TCGC emits a `client-location-conflict` warning if moving those parameters to the same client would result in multiple parameters sharing a name but having different types.

To resolve this, apply `@clientLocation` on each individual operation parameter directly rather than on the shared template definition, so each operation retains its own parameter with a consistent type.
:::
