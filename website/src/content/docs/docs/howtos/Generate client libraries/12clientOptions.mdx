...
| `value`   | `unknown`          | The option value. Primitives (`string`, `boolean`, `number`), arrays, objects, and nested combinations are supported; the consuming emitter determines the expected shape. |
| `scope`   | `string` (optional) | The target language scope. **Required** — omitting scope produces a `client-option-requires-scope` warning.                         |
...
TCGC provides the `getClientOptions` helper function to extract an option value by name from any decorated SDK type.
...
// Get one client option from a model by name
...
The function returns the option value as `unknown`, or `undefined` when the named option is not present. Narrow or validate the value before using it.
